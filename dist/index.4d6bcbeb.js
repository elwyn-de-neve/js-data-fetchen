...
/* ------------------------------------ */ async function fetchJokeByQuery(query) {
    try {
        const response = await _axiosDefault.default.get("https://api.chucknorris.io/jokes/search", {
            params: {
                query: query
            }
        });
...
/* ------------------------------------ */ function getFiveJokes(array) {
    const fiveJokes = [];
    for(let i = 0; i < 5 && array.length; i++)fiveJokes.push(array[i].value);
    return fiveJokes;
}
...
},{"./../utils":"5By4s"}]},["fcvSp","gLLPy"], "gLLPy", "parcelRequire79da")
